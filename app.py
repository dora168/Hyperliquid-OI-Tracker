...
def create_dual_axis_chart(df, symbol):
    ...
    # 🔥 为了适应矮高度，隐藏 X 轴标签
    base = alt.Chart(df).encode(alt.X('index', title=None, axis=None))
...
def render_chart_component(rank, symbol, bulk_data, ranking_data, is_top_mover=False, list_type=""):
    ...
    if raw_df is not None and not raw_df.empty:
        ...
        color_css = "#009900" if pct_change >= 0 else "#D10000"
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px;">
        <div>
            <span style="font-weight: bold; font-size: 16px;">{fire_icon} {rank_str}</span>
            <a href="{coinglass_url}" target="_blank" style="text-decoration:none; color: #333; font-weight: bold; font-size: 16px; margin-left: 5px;">
                {symbol}
            </a>
        </div>
        <div style="color: {color_css}; font-weight: bold; font-size: 16px;">
            {arrow}{pct_change:.2f}%
        </div>
    </div>
    <div style="display: flex; justify-content: space-between; font-size: 12px; color: #666; margin-bottom: 5px;">
        <span>⚡强度: {int_val:.2f}%</span>
        <span>💰流入: +${growth_str}</span>
    </div>
    """

    # 🔥 使用 st.container(border=True) 替代 st.expander，实现不折叠且有边框
