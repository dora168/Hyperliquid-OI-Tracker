...
@st.cache_resource(ttl=3600) # 缓存连接 1 小时
...
def fetch_data_for_symbol(symbol, conn, limit=DATA_LIMIT):
    ...
    try:
        ...
        # 4. 使用 pandas 读取 SQL 查询结果
        df = pd.read_sql(sql_query, conn, params=(symbol, limit))
        df = df.sort_values('time', ascending=True)
        return df

    except Exception as e:
        # 如果查询失败，返回空的 DataFrame
        st.error(f"❌ 查询 {symbol} 数据失败: {e}")
        return pd.DataFrame()
...
def main_app():
    ...
    # 1. 建立数据库连接 (如果连接失败，应用会停止并显示错误)
    conn = get_db_connection()
    
    # 2. 动态获取所有合约列表 (184 个)
    all_symbols = fetch_all_symbols(conn)
    ...
    if selected_symbol:
        ...
        # 4a. 读取数据
        data_df = fetch_data_for_symbol(selected_symbol, conn)
